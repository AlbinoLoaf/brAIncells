...
class CKACalculator:
    ...
    def _init_values(self, all_layer_X, all_layer_Y):
        ...
        curr_hsic_matrix = torch.zeros(self.num_elements).to(self.device)
        curr_self_hsic_x = torch.zeros(1, self.num_layers_X).to(self.device)
        curr_self_hsic_y = torch.zeros(self.num_layers_Y, 1).to(self.device)
